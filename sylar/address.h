...
public:
     ...
     */
    /*
    接收一个 C 风格的原始网络地址（sockaddr*），根据其内容（地址族、IP、端口、路径等），创建一个对应的 C++ 封装对象（如 IPv4Address、IPv6Address 等）
    ，并将原始地址的数据“拷贝保存”到这个新对象的内部成员中，以便后续以面向对象的方式安全、方便地使用。    
    */
     ...
     */
    /*
    Lookup 负责把用户给的“字符串地址”（比如域名或 IP+端口）解析成底层可用的网络地址（通过 DNS 或直接识别），
    然后内部调用 Create 把这些地址包装成 C++ 的 Address 对象，供后续 connect、bind 等系统调用使用。
    */
     ...
     */
    //lookupany函数用于根据给定的主机名（host）和可选的网络参数（family、type、protocol）查找并返回一个符合条件的网络地址对象（Address::ptr）。
     ...
     */
    //LookupAnyIPAddress 解析地址后，确保返回的是一个带端口的 IP 地址对象（IPv4 或 IPv6），方便后续做网络连接或地址计算，避免拿到无效类型。
...
public:
     ...
     */
    //只接受数字ip，不接受域名，最终返回一个子类指针
    /*
    用户调用 IPAddress::Create("192.168.1.1", 8080)
    构造 hints：只接受数值 IP（AI_NUMERICHOST），支持 IPv4/IPv6（AF_UNSPEC）
    调用 getaddrinfo("192.168.1.1", nullptr, &hints, &results)
    成功 → results->ai_addr 指向一个 sockaddr_in（在堆上）
    调用 Address::Create(results->ai_addr, ...)
    内部 switch (sa_family) 判断为 AF_INET
    拷贝 *(sockaddr_in*)ai_addr 的值
    构造 new IPv4Address(拷贝值) → 返回 Address::ptr
    dynamic_pointer_cast<IPAddress> 转换成功 → 得到 IPAddress::ptr
    调用 result->setPort(8080) → 修改对象内部的端口副本
    freeaddrinfo(results) → 安全释放系统分配的链表（不影响已拷贝的对象）
    返回 IPAddress::ptr，其内部数据独立、完整，可长期使用
    */
...
public:
    ...
    const sockaddr* getAddr() const override;//获取sockaddr指针，只读
    sockaddr* getAddr() override;//获取sockaddr指针，读写
    socklen_t getAddrLen() const override;//获取sockaddr长度
    std::ostream& insert(std::ostream& os) const override;//可读性输出地址

    IPAddress::ptr broadcastAddress(uint32_t prefix_len) override;//获取广播地址
    IPAddress::ptr networdAddress(uint32_t prefix_len) override;//获取网段地址
