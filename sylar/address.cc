...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    /*
    struct addrinfo {
    int              ai_flags;     // 输入参数：标志位（如 AI_PASSIVE, AI_CANONNAME）
    int              ai_family;    // 地址族：AF_INET, AF_INET6, AF_UNSPEC 等
    int              ai_socktype;  // 套接字类型：SOCK_STREAM, SOCK_DGRAM 等
    int              ai_protocol;  // 协议：IPPROTO_TCP, IPPROTO_UDP 等
    socklen_t        ai_addrlen;   // ai_addr 指向的地址结构体的长度
    struct sockaddr *ai_addr;      // 指向实际地址（sockaddr_in 或 sockaddr_in6）
    char            *ai_canonname; // 规范主机名（仅当 AI_CANONNAME 设置时有效）
    struct addrinfo *ai_next;      // 指向链表中的下一个 addrinfo 结构
};
    */
    memset(&hints, 0, sizeof(addrinfo));

    hints.ai_flags = AI_NUMERICHOST;//只解析数字地址，不进行域名解析
    hints.ai_family = AF_UNSPEC;//不限协议族，只接受ipv4和ipv6

    //调用getaddrinfo解析地址
    //getaddrinfo的参数，第一个为主机名或许ip地址，第二个为服务名或端口号，第三个为addrinfo用于指定查询条件，第四个为链表结果需要手动freeaddrinfo释放
